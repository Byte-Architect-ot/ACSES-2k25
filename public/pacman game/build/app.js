...
gameOver() {
  ...
  sendScore2fb();
    ...
    if (name && this.highScore  && this.highScore>2500) {
        fetch('/api/score', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name, score: this.highScore })
    })
        .then(response => response.json())
        .then(result => console.log("✅ Score synced:", result.message))
